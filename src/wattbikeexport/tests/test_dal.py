...
def _make_snapshot(root: Path, *, title: str = "Quick Ride") -> None:
    ...
    wbsr_path.write_text(json.dumps({"revolutions": []}))

    (root / "profile.json").write_text(json.dumps({"objectId": "user"}))
    (root / "profile-objects.json").write_text(json.dumps({}))
    (root / "sessions.json").write_text(json.dumps([session]))
